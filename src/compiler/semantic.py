...
def p_funcdef(p: yacc.YaccProduction):
    """FUNCDEF : DEF IDENT new_scope LEFTPARENTHESES PARAMLIST RIGHTPARENTHESES LEFTBRACE STATELIST RIGHTBRACE"""
...
def p_opt_vector(p: yacc.YaccProduction):
    """OPT_VECTOR : LEFTBRACKET INTCONSTANT RIGHTBRACKET OPT_VECTOR
                  | empty
    """
    if len(p) > 2:
        p[0] = [p[2], *p[4]]
...
def p_funccal_or_exp_int_const(p: yacc.YaccProduction):
    """FUNCCALL_OR_EXPRESSION : INTCONSTANT REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR"""
...
def p_funccal_or_exp_float_const(p: yacc.YaccProduction):
    """FUNCCALL_OR_EXPRESSION : FLOATCONSTANT REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR"""
...
def p_funccal_or_exp_string_const(p: yacc.YaccProduction):
    """FUNCCALL_OR_EXPRESSION : STRINGCONSTANT REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR"""
...
def p_funccall_or_exp_parentesis(p: yacc.YaccProduction):
    """FUNCCALL_OR_EXPRESSION : LEFTPARENTHESES NUMEXPRESSION RIGHTPARENTHESES REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR"""
    node = p[2]['node']
...
def p_follow_ident_parentesis(p: yacc.YaccProduction):
    """FOLLOW_IDENT : LEFTPARENTHESES PARAMLISTCALL RIGHTPARENTHESES """
    pass
...
def p_ifstat(p: yacc.YaccProduction):
    """IFSTAT : IF LEFTPARENTHESES EXPRESSION RIGHTPARENTHESES new_scope LEFTBRACE STATELIST RIGHTBRACE OPT_ELSE"""
    # Go back to previous scope
    scope_stack.pop()


def p_opt_else(p: yacc.YaccProduction):
    """OPT_ELSE : ELSE new_scope LEFTBRACE STATELIST RIGHTBRACE
                | empty
    """
    if len(p) > 2:
        # Go back to previous scope
...
def p_forstat(p: yacc.YaccProduction):
    """FORSTAT : FOR LEFTPARENTHESES ATRIBSTAT SEMICOLON EXPRESSION SEMICOLON ATRIBSTAT RIGHTPARENTHESES new_loop_scope LEFTBRACE STATELIST RIGHTBRACE"""
    scope_stack.pop()
...
def p_allocexp(p: yacc.YaccProduction):
    """ALLOCEXPRESSION : NEW DATATYPE LEFTBRACKET NUMEXPRESSION RIGHTBRACKET OPT_ALLOC_NUMEXP"""
    num_expressions.append((p[4]['node'], p.lineno(1)))


def p_opt_allocexp(p: yacc.YaccProduction):
    """OPT_ALLOC_NUMEXP : LEFTBRACKET NUMEXPRESSION RIGHTBRACKET OPT_ALLOC_NUMEXP
                        | empty
    """
    if len(p) < 3:
        p[0] = ''
...
def p_factor_expr(p: yacc.YaccProduction):
    """FACTOR : LEFTPARENTHESES NUMEXPRESSION RIGHTPARENTHESES"""
    p[0] = p[2]
