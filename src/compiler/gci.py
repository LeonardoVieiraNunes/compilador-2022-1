...
def p_funccall_or_exp_string(p: yacc.YaccProduction):
    """FUNCCALL_OR_EXPRESSION : INTCONSTANT REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR
                              | STRINGCONSTANT REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR
                              | FLOATCONSTANT REC_UNARYEXPR REC_PLUS_MINUS_TERM OPT_REL_OP_NUM_EXPR"""
...
def p_ifstat(p: yacc.YaccProduction):
    """IFSTAT : IF LEFTPARENTHESES EXPRESSION RIGHTPARENTHESES LEFTBRACE STATELIST RIGHTBRACE OPT_ELSE"""
...
def p_opt_else(p: yacc.YaccProduction):
    """OPT_ELSE : ELSE LEFTBRACE STATELIST RIGHTBRACE
                | empty
    """
    if len(p) < 3:
        p[0] = {
...
def p_forstat(p: yacc.YaccProduction):
    """FORSTAT : new_for_loop_label FOR LEFTPARENTHESES ATRIBSTAT SEMICOLON EXPRESSION SEMICOLON ATRIBSTAT RIGHTPARENTHESES LEFTBRACE STATELIST RIGHTBRACE"""
    start_label = new_label()
